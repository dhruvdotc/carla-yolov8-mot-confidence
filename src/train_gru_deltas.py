# Train a GRU model to predict FUTURE delta (dx,dy) positions from PAST (x,y)
# Uses dataset.npz created by make_windows.py
# Reports ADE and FDE on train/val/test splits (computed on reconstructed absolute positions)
...
SAVE_WEIGHTS = "C:/Users/dhruv/carla_sim/outputs/bs_conf50_mot/gru_deltas.pt"
...
def future_to_deltas(last_past_xy, future_abs_xy):
    # last_past_xy: (B, 2)
    # future_abs_xy: (B, T, 2)
    # returns deltas: (B, T, 2) where delta[t] = future_abs[t] - anchor
    # anchor is last_past for t=0, then previous future_abs for later steps
    B, T, _ = future_abs_xy.shape
    deltas = torch.zeros_like(future_abs_xy)

    prev = last_past_xy
    for t in range(T):
        deltas[:, t, :] = future_abs_xy[:, t, :] - prev
        prev = future_abs_xy[:, t, :]

    return deltas


def deltas_to_future(last_past_xy, future_deltas_xy):
    # last_past_xy: (B, 2)
    # future_deltas_xy: (B, T, 2)
    # reconstruct absolute future by cumulative sum starting from last_past
    B, T, _ = future_deltas_xy.shape
    future_abs = torch.zeros_like(future_deltas_xy)

    prev = last_past_xy
    for t in range(T):
        prev = prev + future_deltas_xy[:, t, :]
        future_abs[:, t, :] = prev
...
class GRUPredictor(nn.Module):
    # GRU encoder -> MLP head -> outputs future deltas (dx,dy)
...
def run_epoch(model, loader, optimizer=None):
    ...
    for past_xy, future_abs_xy in loader:
        past_xy = past_xy.to(DEVICE)
        future_abs_xy = future_abs_xy.to(DEVICE)

        last_past = past_xy[:, -1, :]  # (B,2)

        # Build delta targets from absolute ground-truth future
        true_deltas = future_to_deltas(last_past, future_abs_xy)

        # Predict deltas
        pred_deltas = model(past_xy)

        # Train loss in delta space (more stable)
        loss = nn.functional.mse_loss(pred_deltas, true_deltas)

        # Evaluate ADE/FDE in absolute space (reconstruct)
        pred_abs = deltas_to_future(last_past, pred_deltas)
        ade, fde = ade_fde(pred_abs, future_abs_xy)
